...
library ThreeHeapOrdering {
    ...
    /// @dev Only call this function when `id` is in the `heap` with value `formerValue` or when `id` is not in the `heap` with `formerValue` equal to 0. Don't call this function with `maxSortedUsers` equal to 0.
    ...
    /// @notice Computes a new suitable size from `size` that is smaller than `maxSortedUsers`.
    ...
    function _computeSize(uint256 size, uint256 maxSortedUsers) private pure returns (uint256) {
        while (size >= maxSortedUsers) size /= 3;
        return size;
    }
    ...
    function _shiftDown(HeapArray storage heap, uint256 size, Account memory accountToShift, uint256 index) private {
        ...
        uint256 targetIndex = index;
        uint256 nextIndex = index * 3;

        while (true) {
            uint256 rightChildIndex = nextIndex + 3;
            while (++nextIndex <= rightChildIndex && nextIndex < size) {
                Account memory nextAccount = heap.accounts[nextIndex];
            ...
            _setAccount(heap, targetAccount, index);
    ...
    function _decrease(HeapArray storage heap, uint256 size, address id, uint96 newValue) private {
        ...
        if (3 * index + 1 < size) _shiftDown(heap, size, Account(id, newValue), index);
