...
library LogarithmicBuckets {
    ...
    function remove(BucketList storage _buckets, address _id) internal {
    ...
    function getValueOf(BucketList storage _buckets, address _id) internal view returns (uint256) {
        uint256 index = _buckets.indexOf[_id];
        DoubleLinkedList.Account memory account = _buckets.lists[index].accounts[_id];
    ...
    /// @param _buckets The heap to get the head.
